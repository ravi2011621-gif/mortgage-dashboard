...
function TrendPill({ current, previous }) {
  ...
  if (trend.direction === "up") {
    return <div style={{ display: "inline-flex", alignItems: "center", gap: 4, borderRadius: 999, background: "#fff1f2", color: "#be123c", padding: "6px 10px", fontSize: 12, fontWeight: 600 }}><ArrowUpRight size={12} />+{trend.delta}%</div>;
  }
  if (trend.direction === "down") {
    return <div style={{ display: "inline-flex", alignItems: "center", gap: 4, borderRadius: 999, background: "#ecfdf5", color: "#047857", padding: "6px 10px", fontSize: 12, fontWeight: 600 }}><ArrowDownRight size={12} />{trend.delta}%</div>;
  }
    ...
    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {RANGE_OPTIONS.map((option) => (
          <button key={option.key} onClick={() => setRange(option.key)} style={{ border: "none", cursor: "pointer", borderRadius: 999, padding: "8px 14px", fontSize: 14, fontWeight: 600, background: range === option.key ? "#0f172a" : "#f1f5f9", color: range === option.key ? "#ffffff" : "#334155", transition: "all 180ms ease" }}>
            {option.label}
          </button>
        ))}
      </div>
...
function NoDataBox({ text = "No chart data available for the selected range." }) {
  return (
    <div style={{ height: 420, display: "flex", alignItems: "center", justifyContent: "center", border: "1px dashed #cbd5e1", borderRadius: 18, color: "#64748b", fontSize: 14, background: "#fafafa" }}>
      {text}
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #f8fafc 0%, #eef2ff 100%)", padding: 24, color: "#0f172a" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <SimpleCard style={{ padding: 24 }}>
            <div style={{ display: "flex", gap: 16, justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap" }}>
              <div>
                ...
                <p style={{ marginTop: 10, marginBottom: 0, fontSize: 15, color: "#64748b" }}>Server-side FRED data, richer Fed meeting analysis, mortgage spreads, yield charts, and stronger visual polish.</p>
              </div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {[["rates", "Rates"], ["macro", "Macro"], ["meetings", "Fed meetings"], ["analysis", "AI Economist Review"]].map(([id, label]) => (
            <a key={id} href={`#${id}`} style={{ display: "inline-block", borderRadius: 999, background: "#0f172a", color: "#ffffff", padding: "10px 16px", textDecoration: "none", fontSize: 14, fontWeight: 600 }}>{label}</a>
          ))}
        </div>
            ...
            <SimpleCard>
              ...
              <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
                {analysis.factors.map((factor) => (
                  <div key={factor.name}>
                    <div style={{ marginBottom: 8, display: "flex", justifyContent: "space-between", gap: 10, fontSize: 14 }}><span style={{ fontWeight: 600, color: "#334155" }}>{factor.name}</span><span style={{ color: "#64748b" }}>{factor.score}/100</span></div>
              ...
              <div style={{ maxHeight: 620, overflow: "auto", padding: 20, paddingTop: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {FOMC_MEETINGS.map((meeting) => {
                    ...
                    return (
                      <button key={meeting.date} onClick={() => setSelectedMeeting(meeting)} style={{ width: "100%", borderRadius: 18, border: `1px solid ${active ? "#0f172a" : "#e2e8f0"}`, background: active ? "#0f172a" : "#f8fafc", color: active ? "#ffffff" : "#0f172a", padding: 16, textAlign: "left", cursor: "pointer", transition: "all 180ms ease" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                          ...
                          <div style={{ borderRadius: 999, padding: "6px 10px", fontSize: 12, fontWeight: 700, background: active ? "#ffffff" : "transparent", color: active ? "#0f172a" : "#475569", border: active ? "none" : "1px solid #cbd5e1" }}>{meeting.rateChange}</div>
                        </div>
                      </button>
                    );
                  })}
                  ...
                  <motion.div key={selectedMeeting?.date} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }} style={{ display: "flex", flexDirection: "column", gap: 16, borderRadius: 24, border: "1px solid #e2e8f0", background: "#f8fafc", padding: 20 }}>
                    ...
                    <div>
                      ...
                      <ul style={{ marginTop: 8, paddingLeft: 20, color: "#334155" }}>
                        {selectedMeeting?.keyPoints?.map((point) => <li key={point} style={{ marginBottom: 8, lineHeight: 1.6 }}>{point}</li>)}
                      </ul>
                    </div>
          ...
          <SimpleCard>
            ...
            <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 20 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
                <motion.div whileHover={{ y: -3 }} style={{ borderRadius: 18, border: "1px solid #e2e8f0", background: "#f8fafc", padding: 20 }}>
                  ...
                  <div style={{ marginTop: 10, display: "inline-flex", borderRadius: 999, background: "#0f172a", color: "#ffffff", padding: "6px 12px", fontSize: 12, fontWeight: 700 }}>{analysis.tone}</div>
                </motion.div>
                <motion.div whileHover={{ y: -3 }} style={{ borderRadius: 18, border: "1px solid #e2e8f0", background: "#f8fafc", padding: 20 }}>
                ...
                <motion.div whileHover={{ y: -3 }} style={{ borderRadius: 18, border: "1px solid #e2e8f0", background: "#f8fafc", padding: 20 }}>
                  ...
                  <div style={{ marginTop: 10 }}><TrendPill current={analysis.mortgageSpread} previous={analysis.previousSpread} /></div>
                </motion.div>
              </div>
              ...
              <div style={{ borderRadius: 24, border: "1px solid #e2e8f0", background: "#ffffff", padding: 20 }}>
                ...
                <p style={{ marginTop: 12, marginBottom: 0, fontSize: 14, lineHeight: 1.8, color: "#334155" }}>{analysis.narrative}</p>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16 }}>
                {analysis.factors.map((factor) => (
                  <motion.div whileHover={{ y: -3 }} key={factor.name} style={{ borderRadius: 18, border: "1px solid #e2e8f0", background: "#ffffff", padding: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center" }}>
                      <div style={{ fontWeight: 700 }}>{factor.name}</div>
                      <div style={{ fontSize: 13, color: "#64748b" }}>{factor.score}/100</div>
                    </div>
                    <div style={{ marginTop: 12 }}><HeatBar value={factor.score} /></div>
                    <div style={{ marginTop: 12, fontSize: 13, color: "#64748b" }}>{factor.level != null ? `Current: ${formatPct(factor.level)}` : "Qualitative factor"}</div>
                    <p style={{ marginTop: 12, marginBottom: 0, fontSize: 14, lineHeight: 1.7, color: "#334155" }}>{factor.detail}</p>
                  </motion.div>
                ))}
              </div>
            </div>
